...
async function quoteRoutes(app) {
    // ─── CLIENTS (mapeados dos dados do quote) ────────────────────────────────
    app.get('/clients', async (_req, reply) => {
        const clients = quote_store_1.quoteStore.findAll().map((q) => ({
            id: q.id,
            name: q.clientName,
            email: q.clientEmail,
    ...
    app.get('/clients/:id', async (req, reply) => {
        const q = quote_store_1.quoteStore.findById(req.params.id);
        if (!q)
    ...
    app.get('/quotes', async (_req, reply) => {
        return reply.send(quote_store_1.quoteStore.findAll().map(toApiQuote));
    });
    app.get('/quotes/metrics/by-service', async (_req, reply) => {
        const all = quote_store_1.quoteStore.findAll();
        const map = {};
        for (const q of all)
            map[q.serviceType] = (map[q.serviceType] ?? 0) + 1;
    ...
    app.get('/quotes/:id/history', async (req, reply) => {
        const q = quote_store_1.quoteStore.findById(req.params.id);
        if (!q)
            return reply.status(404).send({ error: 'Quote not found' });
    ...
    app.post('/quotes/update-status', async (req, reply) => {
        const { id, status } = req.body;
        const updated = quote_store_1.quoteStore.patch(id, { status });
        if (!updated)
            return reply.status(404).send({ error: 'Quote not found' });
        quote_observer_1.quoteObserver.notifyUpdated(updated);
        return reply.send(toApiQuote(updated));
    });
    app.post('/quotes/update-asaas', async (req, reply) => {
        const { id } = req.body;
        const q = quote_store_1.quoteStore.findById(id);
        if (!q)
            return reply.status(404).send({ error: 'Quote not found' });
        return reply.send(toApiQuote(q));
    });
    app.post('/quotes/delete', async (req, reply) => {
        quote_store_1.quoteStore.patch(req.body.id, {});
        return reply.send({ success: true });
    });
    app.post('/quotes/pdf', async (req, reply) => {
        const pdfBuffer = await (0, pdf_service_1.generateProposalPdf)(req.body);
        reply
    ...
    app.post('/settings/upsert', async (req, reply) => {
    ...
    app.post('/settings/delete', async (req, reply) => {
    ...
    app.post('/prospects/update', async (req, reply) => {
        const { id } = req.body;
    ...
    app.post('/prospects/delete', async (req, reply) => {
    ...
    app.get('/agents/:id', async (_req, reply) => reply.send(agentStore));
    app.post('/agents', async (req, reply) => {
        agentStore = { ...agentStore, ...req.body, id: (0, crypto_1.randomUUID)(), updated_at: new Date().toISOString() };
        return reply.status(201).send(agentStore);
    });
    app.post('/agents/update', async (req, reply) => {
        agentStore = { ...agentStore, ...req.body, updated_at: new Date().toISOString() };
